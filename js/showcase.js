(function (){
  ...
  function App () {
    ...
    $private.writesItemData = function (item) {
      ...
      item.dom.querySelector('.payment-conditions').textContent = item.data.productInfo.paymentConditions;
      if ('oldPrice' in item.data)
        item.dom.querySelector('.old-price span').textContent = item.data.oldPrice;
    } 
